...
export class Attachment {
  ...
  public get filename() {
    return this.targetPath.fullName;
  }

  public get basename() {
    return this.targetPath.basename;
  }

  public get extension() {
    return this.targetPath.extension;
  }

  public get extensionName() {
    return this.targetPath.extensionName;
  }
  ...
  public get sourcePathRootRelative() {
  ...
  public set data(data: string | Buffer) {
    this._data = data;
    if (!this.source) {
      this.sourceStat = { ctime: Date.now(), mtime: Date.now(), size: this.data?.length ?? 0 };
    }
  }
  ...
  constructor(data: string | Buffer, target: Path, source: TFile | undefined | null, options: ExportPipelineOptions) {
    ...
    if (target.isDirectory) {
      throw new Error("target must be a file: " + target.path);
    }
    if (target.isAbsolute) {
      throw new Error("(absolute) Target must be a relative path with the working directory set to the root: " + target.path);
    }
