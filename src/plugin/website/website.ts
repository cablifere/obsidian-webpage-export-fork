...
export class Website {
  ...
  constructor(destination: Path | string, options?: ExportPipelineOptions) {
    ...
    if (!destination.isDirectoryFS) {
      throw new Error("Website destination must be a folder: " + destination.path);
    }
  ...
  public async load(files?: TFile[]): Promise<this> {
    ...
    const rootPath = this.findCommonRootPath(this.sourceFiles);
    ...
    for (const file of this.sourceFiles) {
      try {
        const webpage = new Webpage(file, file.name, this, this.exportOptions);
  ...
  public async build(files?: TFile[]): Promise<Website | undefined> {
    ...
    const webpages = this.index.webpages;
  ...
  public async createAttachmentFromSrc(src: string, sourceFile: TFile): Promise<Attachment | undefined> {
    ...
    if (!attachment.sourcePath) {
      attachment.sourcePath = attachedFile.pathname;
    }
    return attachment;
  }
