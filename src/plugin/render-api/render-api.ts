...
export namespace MarkdownRendererAPI {
  ...
  export function extensionToTag(extension: string) {
    if ([ "png", "jpg", "jpeg", "svg", "gif", "bmp", "ico" ].includes(extension)) {
      return "img";
    } else if ([ "mp4", "mov", "avi", "webm", "mpeg" ].includes(extension)) {
      return "video";
  ...
  export async function renderFile(file: TFile, options?: MarkdownRendererOptions): Promise<{ contentEl: HTMLElement, viewType: string } | undefined> {
    ...
    await _MarkdownRendererInternal.postProcessHTML(result.contentEl);
    return result;
  }
  ...
  export async function beginBatch() {
    await _MarkdownRendererInternal.beginBatch();
  }
...
export namespace _MarkdownRendererInternal {
  export const overlayProgress: boolean = true;
  ...
  function failRender(file: TFile | undefined, message: string): undefined {
    if (checkCancelled()) {
      return undefined;
    }
  ...
  export async function renderFile(file: TFile, options: MarkdownRendererOptions): Promise<{ contentEl: HTMLElement, viewType: string } | undefined> {
    ...
    try {
      ...
      return failRender(file, e.message);
    }
  ...
  export async function renderMarkdownView(preview: MarkdownPreviewView, options: MarkdownRendererOptions): Promise<HTMLElement | undefined> {
    ...
    await waitUntil(
      () =>
        !preview.containerEl.querySelector("[class^='block-language-']:empty") || checkCancelled(),
      2000,
      16,
    );
    if (checkCancelled()) {
    ...
    for (const block of invalidPluginBlocks) {
      ExportLog.warning(`Plugin element ${
        block.className
    ...
    for (const canvas of canvases) {
      ...
      const canvasSuccess = await waitUntil(
    ...
    if (options.unifyTitleFormat) {
      const title = await _MarkdownRendererInternal.getTitleForFile(preview.file);
      const icon = await _MarkdownRendererInternal.getIconForFile(preview.file);
      const iconSVG
  ...
  export async function getIconForFile(file: TAbstractFile): Promise<{ icon: string, isDefault: boolean }> {
    ...
    if (useFile instanceof TFolder) {
      ...
      const childFile = ObsidianApp.app.vault.getFileByPath(file.path + "/" + file.name + ".md");
      if (childFile) {
        useFile = childFile;
      }
    }
    ...
    if ((useDefaultIcon || !iconProperty || isUnchangedNotEmojiNotHTML) && app?.plugins?.enabledPlugins?.has("obsidian-icon-folder")) {
      ...
      // because that is what we rely on to get icon
      if (noteIconsEnabled) {
        ...
        if (iconProperty && typeof iconProperty === "string" && iconProperty.trim() !== "") {
          ...
          const emojiMatch = iconProperty.trim().match(/\p{Emoji}/u);
          const isEmoji = emojiMatch && emojiMatch.length === 1 && emojiMatch.index === 0;

          if (isEmoji) {
            iconOutput = iconProperty;
          } else {
    ...
    if (!parsedAsIconize && isUnchangedNotEmojiNotHTML) {
      iconOutput = "";
    }
  ...
  export async function getTitleForFile(file: TAbstractFile): Promise<{ title: string, isDefault: boolean }> {
    ...
    if (file instanceof TFile) {
      ...
      const frontmatter = fileCache?.frontmatter;
      const titleFromFrontmatter = frontmatter?.[Settings.exportOptions.titleProperty] ?? frontmatter?.["banner_header"]; // banner plugin support
  ...
  export async function createMediaPage(file: TFile, options: MarkdownRendererOptions): Promise<HTMLElement> {
    ...
    const media = contentEl.createEl(embedType);
  ...
  export async function postProcessHTML(html: HTMLElement) {
    ...
    html.querySelectorAll("p:has(> .inline-embed.markdown-embed)").forEach((element) => {
      ...
      element.remove();
      const embed = span.querySelector(".inline-embed.markdown-embed") as HTMLElement;
  ...
  function generateLogEl(title: string, message: string | Error, textColor: string, backgroundColor: string): HTMLElement {
  ...
  function createLoadingContainer() {
    if (!loadingContainer) {
      ...
      const cancelButton = loadingContainer.querySelector(".html-progress-cancel") as HTMLButtonElement;
  ...
  export async function _reportError(messageTitle: string, message: string | Error, fatal: boolean) {
    if (!batchStarted) {
      return;
    }
  ...
  export async function _reportWarning(messageTitle: string, message: string | Error) {
    if (!batchStarted) {
      return;
    }
  ...
  export async function _reportInfo(messageTitle: string, message: string | Error) {
    if (!batchStarted) {
      return;
    }
...
export namespace ExportLog {
  ...
  function humanReadableJSON(object: object) {
  ...
  export function log(message: string | Error, messageTitle: string = "") {
    pullPathLogs();
    ...
    if (SettingsPage.loaded && !(Settings.logLevel === "all")) {
      return;
    }
  ...
  export function warning(message: any, messageTitle: string = "") {
    ...
    if (SettingsPage.loaded && ![ "warning", "all" ].contains(Settings.logLevel)) {
      return;
    }
  ...
  export function error(message: string | Error, messageTitle: string = "", fatal: boolean = false) {
    pullPathLogs();
    ...
    if (SettingsPage.loaded && !fatal && ![ "error", "warning", "all" ].contains(Settings.logLevel)) {
      return;
    }
