...
export class AssetLoader extends Attachment {
  ...
  constructor(filename: string, data: string | Buffer, source: TFile | undefined | null, type: AssetType, inlinePolicy: InlinePolicy, minify: boolean, mutability: Mutability, loadMethod: LoadMethod = LoadMethod.Async, loadPriority: number = 100, cdnPath: string | undefined = undefined, options: ExportPipelineOptions | undefined = undefined) {
    if (source && options === undefined) {
      throw new Error("WebAsset options cannot be empty if source is not empty");
    }
    ...
    if (mutability !== Mutability.Child) {
  ...
  public static extensionToType(extension: string): AssetType {
    ...
    if (mediaTypes.includes(extension)) {
      return AssetType.Media;
    }
    if (scriptTypes.includes(extension)) {
      return AssetType.Script;
    }
    if (styleTypes.includes(extension)) {
      return AssetType.Style;
    }
    if (htmlTypes.includes(extension)) {
      return AssetType.HTML;
    }
    if (fontTypes.includes(extension)) {
  ...
  async minifyAsset() {
    if (this.type === AssetType.HTML || typeof this.data !== "string") {
      return;
    }
    ...
    try {
      ...
      if (isJS) {
        tempContent = `<script>${tempContent}</script>`;
      }
      if (isCSS) {
        tempContent = `<style>${tempContent}</style>`;
      }
      ...
      console.error(`Unable to minify ${(isJS ? "JS" : "CSS")} file.`, e);
    ...
    if (this.isRefFormat(options)) {
      ...
      switch (this.type) {
        ...
        case AssetType.Media:
          ...
          if (this.loadMethod == LoadMethod.Default) {
            attr = "";
          }
  ...
  public static extensionToMime(extension: string): string {
    ...
    return FileMimeType[extension] || "application/octet-stream";
  }
